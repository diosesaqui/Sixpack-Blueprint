...
class ExercisePreviewRouter: NSObject, ExercisePreviewRoutingLogic, ExercisePreviewDataPassing {
    ...
    var workout: Workout? // Store custom workout if provided
    ...
    func passDataToWorkout(source: ExercisePreviewDataStore?, destination: inout (WorkoutBusinessLogic & WorkoutDataStore)?) {
        ...
        // Use the custom workout if provided, otherwise create one
        if let customWorkout = workout {
            destination?.workout = customWorkout
        } else if let exercises = source?.exercises, !exercises.isEmpty {
